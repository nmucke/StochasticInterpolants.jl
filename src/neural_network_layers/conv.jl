...
function (unet::UNet)(
    x::Tuple{AbstractArray{AbstractFloat, 4}, AbstractArray{Any, 4}}, 
    ps::NamedTuple,
    st::NamedTuple
)
...
function (conditional_unet::ConditionalUNet)(
    ...
    emb = reshape(emb, 1, 1, size(emb)[end-1], size(emb)[end])
    @assert size(emb)[[1, 2, 4]] == (1, 1, size(noise_variances, 4))
...
# function (conditional_trans_unet::ConditionalTransformerUNet)(
#     x,#::Tuple{AbstractArray{T, 4}, AbstractArray{T, 4}, AbstractArray{S, 4}}, 
#     ps::NamedTuple,
#     st::NamedTuple
# )# where T <: AbstractFloat
